...
class Image < Helper::Drawable
  ...
  #Imageの指定座標への色の取得／設定
  def _PIXEL_(yield_stack, x:, y:, color:)
    if color
      @entity[x, y] = color
    end
    if block
      #ブロックが付与されているならそれを実行する
      unshift_command_block(@temp_command_block, yield_stack, @entity[x, y], nil)
    end
  end

  #Imageを指定座標の色を比較し、同値ならブロックを実行する
  def _COMPARE_(yield_stack, x:, y:, color:)
  ...
  #指定したツリーを描画する
  def _DRAW_(yield_stack, _ARGUMENT_: , x: 0, y: 0, scale: nil, **)
    #中間バッファを生成
